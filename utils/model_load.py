...
def load_model_weights(model, ckpt_path, use_ema=True, device=None):
    ...
    # checkpoint = torch.load(ckpt_path,map_location={'cuda:3': str(device)})
...
def load_lora_weight(model,lora_path,use_ema,device='cuda'):
    ...
    if "model_ema" in checkpoint and use_ema:
        ...
        if ('module' in ema_key) or ('n_averaged' in ema_key):
            model = model.module
            print(f'\nLoading EMA module model from {lora_path} with {total_iter} iterations')
        else:
            print(f'\nLoading EMA model from {lora_path} with {total_iter} iterations')
    else:
