...
if __name__ == '__main__':
    ...
    assert opt.dataset_name == 't2m' or 'kit' or 'cmp'
    ...
    # generate
    opt.footskate_cleanup=False
    pred_motions,pred_mid= pipeline.generate(texts, torch.LongTensor([int(x) for x in motion_lens]),footskate_cleanup = opt.footskate_cleanup)
    ...
    for i, motion in enumerate(pred_motions):
        motion = motion.squeeze().cpu().numpy() * std + mean
    ...
    for i,title in enumerate(texts):
        motion = samples[i]
        fname = f'{i:02}.mp4'
        kinematic_tree = paramUtil.t2m_kinematic_chain if (opt.dataset_name == 't2m' or opt.dataset_name == 'cmp') else paramUtil.kit_kinematic_chain
        plot_3d_motion(pjoin(out_path, fname), kinematic_tree, motion, title=title, fps=opt.fps, radius=opt.radius)

    for i, motion in enumerate(pred_mid):
        motion = motion.squeeze().cpu().numpy() * std + mean
        npy_name = f'{i:02}.npy'
        # 1. recover 3d joints representation by ik
        motion = recover_from_ric(torch.from_numpy(motion).float(), opt.joints_num)
        ...
        motion = motion_temporal_filter(motion, sigma=1)
        # 4. save
        np.save(pjoin(npy_dir, f'{npy_name}_progress_{i}'), motion)
    

        fname = f'progress_{i:02}.mp4'
        kinematic_tree = paramUtil.t2m_kinematic_chain if (opt.dataset_name == 't2m' or opt.dataset_name == 'cmp') else paramUtil.kit_kinematic_chain
        plot_3d_motion(pjoin(out_path, fname), kinematic_tree, motion, title=f'{texts[0]}', fps=opt.fps, radius=opt.radius)
