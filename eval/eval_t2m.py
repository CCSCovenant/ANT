...
# ---------------------------
def get_co_embeddings_2(captions, motions, model_path="/data/kuimou/SET/checkpoints/clip_motion_align_epoch_21.pt"):
    """
    参数：
...
def compute_mmd(x, y, sigma=10,
    ...
    x_norm = np.sum(x ** 2, axis=1,
                    keepdims=True)  # 形状 (n, 1)
    y_norm = np.sum(y ** 2, axis=1,
                    keepdims=True)  # 形状 (m, 1)

    # 计算核矩阵 K_xx, K_yy, K_xy
    K_xx = np.exp(-gamma * (
            x_norm + x_norm.T - 2 * np.dot(
        x, x.T)))  # 形状 (n, n)
    K_yy = np.exp(-gamma * (
            y_norm + y_norm.T - 2 * np.dot(
        y, y.T)))  # 形状 (m, m)
    K_xy = np.exp(-gamma * (
            x_norm + y_norm.T - 2 * np.dot(
        x, y.T)))  # 形状 (n, m)
    ...
    # 计算无偏估计器的均值（排除对角线元素）
    mean_K_xx_no_diag = (np.sum(
        K_xx) - np.trace(K_xx)) / (
                                n * (
                                n - 1)) if n > 1 else 0
    mean_K_yy_no_diag = (np.sum(
        K_yy) - np.trace(K_yy)) / (
                                m * (
                                m - 1)) if m > 1 else 0
    ...
    # 计算无偏 MMD 值
    mmd_val = scale * (
            mean_K_xx_no_diag + mean_K_yy_no_diag - 2 * mean_K_xy)
    return mmd_val
...
def calculate_transport_score(
    ...
    """
    # 确保特征已归一化
    motion_embs = torch.from_numpy(motion_embs)
    text_embs = torch.from_numpy(text_embs)

    motion_embs = F.normalize(
        motion_embs, dim=1)
    ...
    # 计算余弦相似度和成本矩阵
    cos_sim = torch.mm(motion_embs,
                        text_embs.t()).cpu().numpy()
    ...
    n_motions, n_texts = \
        motion_embs.shape[0], \
            text_embs.shape[0]
    a = np.ones(
        n_motions) / n_motions  # 源分布
    b = np.ones(
        n_texts) / n_texts  # 目标分布

    # 使用Sinkhorn算法求解最优传输计划
    transport_plan = ot.sinkhorn(a, b, cost_matrix, reg, numItermax = 1000)

    # 计算最终传输得分
    transport_score = np.sum(
        transport_plan * cost_matrix)
...
def evaluate_mmd(groundtruth_loader, activation_dict, eval_wrapper, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            ...
            gt_motion_embeddings.append(motion_embeddings.cpu().numpy())
            gt_text_embeddings.append(motion_embeddings.cpu().numpy())
    gt_motion_embeddings = np.concatenate(gt_motion_embeddings, axis=0)
    gt_text_embeddings = np.concatenate(gt_text_embeddings, axis=0)
    mmd_dict = OrderedDict({})
    ots_dict = OrderedDict({})
    for model_name, motion_embeddings in activation_dict.items():
        mmd_val = compute_mmd(gt_motion_embeddings, motion_embeddings, sigma=10, scale=1000)
        ots_val = 0 #calculate_transport_score(gt_motion_embeddings,gt_text_embeddings)
        mmd_dict[model_name] = mmd_val
        ots_dict[model_name] = ots_val
        print(f'---> [{model_name}] Motion MMD: {mmd_val:.4f}')
        print(f'---> [{model_name}] Motion MMD: {mmd_val:.4f}', file=file, flush=True)
        print(f'---> [{model_name}] OTS: {ots_val:.4f}')
        print(f'---> [{model_name}] OTS: {ots_val:.4f}', file=file, flush=True)
    return mmd_dict,ots_dict
...
def evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times,
    ...
    with open(log_file, 'a') as f:
        ...
        for replication in range(replication_times):
            ...
            # 计算 Motion MMD
            mmd_dict,ots_dict = evaluate_mmd(gt_loader, activation_dict, eval_wrapper, f)
            ...
            for model_name, ots_value in ots_dict.items():
                if model_name not in all_metrics['OTS']:
                    all_metrics['OTS'][model_name] = []
                all_metrics['OTS'][model_name].append(ots_value)
        ...
        # 输出 Motion MMD
        for model_name, values in all_metrics['OTS'].items():
            values = np.array(values)
            mean = np.mean(values, axis=0)
            std = np.std(values, axis=0)
            conf_interval = 1.96 * std / np.sqrt(replication_times)
            
            final_metrics[f'OTS_{model_name}'] = mean
            line = f'---> [{model_name}] OTS Mean: {mean:.4f} CInterval: {conf_interval:.4f}'
            print(line)
            print(line, file=f, flush=True)
