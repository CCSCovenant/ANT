...
class Text2MotionDataset(data.Dataset):
    ...
    def __init__(self, opt, split, mode='train', accelerator=None):
        ...
        if mode == 'eval':
            ...
            #  self.mean_for_eval = np.load(pjoin(self.data_root, 'Mean.npy'))
            #  self.std_for_eval = np.load(pjoin(self.data_root, 'Std.npy'))
        ...
        # caption_length_maen=sum/len(data_dict)
...
class CMP(Text2MotionDataset):
    def __init__(self, opt, split="train", mode='train', accelerator=None):
        self.data_root = './data/CMP'
        self.min_motion_len = 40
        self.joints_num = 22
        self.dim_pose = 263
        self.max_motion_length = 196
        if accelerator:
            accelerator.print('\n Loading %s mode CMP %s dataset ...' % (mode,split))
        else:
            print('\n Loading %s mode HumanML3D dataset ...' % mode)
        super(CMP, self).__init__(opt, split, mode, accelerator)
